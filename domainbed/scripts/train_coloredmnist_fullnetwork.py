    ...
    for step in range(flags.steps):
        for edx, env in enumerate(envs):
            ...
            if edx in [0, 1]:
                ...
                optimizer.zero_grad()
                if ("alllayers" in flags.algorithm.split("_") or "onlyextractor" in flags.algorithm.split("_")):
                    env["grad_statistics"] = compute_grad_variance(
                        mlp.prepare_input(env["images"]),
                        env['labels'], mlp.alllayers
                    )
                else:
        ...
        dict_grad_statistics_averaged = {
            name:
            torch.stack([envs[0]["grad_statistics"][name], envs[1]["grad_statistics"][name]],
                        dim=0).mean(dim=0) for name in envs[0]["grad_statistics"]
        }
        fishr_penalty = (
