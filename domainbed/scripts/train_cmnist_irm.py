    ...
    def compute_grads_variance(features, labels, classifier):
        ...
        for name, _grads in dict_grads.items():
            ...
            if flags.algorithm == "fishr_offdiagonal":
                dict_grads_variance[name] = torch.einsum(
                    "na,nb->ab", grads, grads) / (grads.size(0) * grads.size(1))
            else:
    ...
    for step in range(flags.steps):
        for edx, env in enumerate(envs):
            ...
            if edx in [0, 1]:
                ...
                optimizer.zero_grad()
                env["grads_variance"] = compute_grads_variance(
        ...
        dict_grads_variance_averaged = OrderedDict(
            [
                (
                    name,
                    torch.stack(
                        [envs[0]["grads_variance"][name], envs[1]["grads_variance"][name]], dim=0
                    ).mean(dim=0)
