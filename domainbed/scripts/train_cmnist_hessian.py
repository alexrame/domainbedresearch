...
parser.add_argument(
    ...
    type=str,
    default="fishr",
    choices=['erm', 'irm', "rex", 'fishr', "erm_fullnetwork", "fishr_fullnetwork"]
)
...
for restart in range(flags.n_restarts):
    ...
    # def compute_grads_cov(features, labels, classifier):
    ...
    #     for n, _grads in dict_grads.items():
    #         grads = _grads
    #         env_mean = grads.mean(dim=0, keepdim=True)
    #         grads = grads - env_mean

    #         batchgrads_cov = (grads).pow(2)
    #         dict_grads_cov[n] = batchgrads_cov.mean(dim=0)
    #     return dict_grads_cov


    # def compute_hessian(features, labels, classifier, diagonal=True, num_splits=0):
    #     assert diagonal
    #     # due to our reliance on backpack and DiagHessian
    #     logits = classifier(features.detach())
    #     assert flags.output_size == 2
    ...
    #     dict_hessian = OrderedDict(
    #         {
    #             n: p.diag_h.clone().view(p.diag_h.size(0), -1)
    #             for n, p in classifier.named_parameters()
    #         }
    #     )
    #     return dict_hessian
    ...
    for step in range(flags.steps):
        ...
        if flags.algorithm in ["erm", "erm_fullnetwork"]:
            pass
        else:
        ...
        if step % 100 == 0:
            ...
            pretty_print(
                ...
                fishr_norm.detach().cpu().numpy(),
                fishr_intradomain_penalty.detach().cpu().numpy(),
                rex_penalty.detach().cpu().numpy(),
                irm_penalty.detach().cpu().numpy(),
                test_acc.detach().cpu().numpy(),
                grayscale_test_acc.detach().cpu().numpy(),
                hessian_penalty.detach().cpu().numpy() if flags.verbose else "",
                hessian_norm.detach().cpu().numpy() if flags.verbose else "",
                hessian_meannorm.detach().cpu().numpy() if flags.verbose else "",
        ...
        if flags.plot:
            ...
            all_fishr_penalties[restart, step] = fishr_penalty.detach().cpu().numpy()
            all_fishr_norm[restart, step] = fishr_norm.detach().cpu().numpy()
            all_fishr_intradomain_penalties[restart, step] = fishr_intradomain_penalty.detach().cpu(
            ).numpy()
            if flags.verbose:
                all_hessian_penalties[restart, step] = hessian_penalty.detach().cpu().numpy()
                all_hessian_norm[restart, step] = hessian_norm.detach().cpu().numpy()
                all_hessian_meannorm[restart, step] = hessian_meannorm.detach().cpu().numpy()
                all_hessian_intradomain_penalties[
                    restart, step] = hessian_intradomain_penalty.detach().cpu().numpy()
