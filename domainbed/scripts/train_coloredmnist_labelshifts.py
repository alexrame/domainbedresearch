...
for restart in range(flags.n_restarts):
    ...
    def make_environment(
        images, labels, e, grayscale=False, accepted_label=None,
    ):
        ...
        if flags.label_flipping_prob:
            labels = torch.remainder(
                labels + 5 * torch_bernoulli(flags.label_flipping_prob, len(labels)), 10)
        ...
        dict_output = {'images': (images.float() / 255.).cuda(), 'labels': labels[:, None].cuda()}
        # pdb.set_trace()
        if accepted_label is not None:
            idx = sum(
                [labels == int(_accepted_label) for _accepted_label in accepted_label.split("_")]
            ).bool()
            dict_output = {key: value[idx] for key, value in dict_output.items()}
        return dict_output

    envs = [
        make_environment(mnist_train[0][::2], mnist_train[1][::2], 0.2, accepted_label="0_1_2_5_6"),
    ...
    class MLP(nn.Module):
        ...
        def __init__(self):
            ...
            self.classifier = extend(nn.Linear(flags.hidden_dim, 10))
    ...
    def mean_accuracy(logits, y, verbose=False):
        pred = torch.argmax(logits, axis=1)
        acc = torch.mean((pred == y.view(-1)).float())
        return acc
    ...
    pretty_print(
        'step', 'train nll', 'train0 acc', 'train1 acc', 'fishr penalty', 'rex penalty',
        'irm penalty',
        'test acc',
        '0_1_2_5_6',
        "3_4_7_8_9"
    ...
    for step in range(flags.steps):
        for edx, env in enumerate(envs):
            ...
            env['acc'] = mean_accuracy(logits, env['labels'], verbose=step==200)
            if edx in [0, 1]:
                ...
                if "classcond" in flags.algorithm.split("_"):
                    ...
                    for label in [0, 1, 2, 5, 6]:
                        ...
                        env['nllrex'][label] = mean_nll(logits[idx], env['labels'][idx])
                        ...
                        if "fishr" in flags.algorithm.split("_"):
                            if "features" in flags.algorithm.split("_"):
                                env["grad_variance"][label] = compute_grad_variance(
        ...
        if "classcond" in flags.algorithm.split("_"):
            ...
            for label in [0, 1, 2, 5, 6]:
                ...
                rex_penalty += (envs[0]['nllrex'][label].mean() - envs[1]['nllrex'][label].mean())**2
                if "fishr" in flags.algorithm.split("_"):
                    dict_grad_variance_averaged = {
                        name: torch.stack(
                            [
                                envs[0]["grad_variance"][label][name],
                                envs[1]["grad_variance"][label][name]
                            ],
                            dim=0
                    ...
                    fishr_penalty += (
                        l2_between_grad_variance(
                            envs[0]["grad_variance"][label], dict_grad_variance_averaged
                        ) + l2_between_grad_variance(
                            envs[1]["grad_variance"][label], dict_grad_variance_averaged
                        )
                    )
                else:
